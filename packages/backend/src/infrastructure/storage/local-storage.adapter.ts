...
export class LocalStorageAdapter implements StoragePort {
   ...
   * Validates the resolved path stays within uploadDir to prevent path traversal.
   */
  ...
  /**
   * Resolves a file key to an absolute path and ensures it stays within
   * the uploadDir. Prevents path traversal attacks (e.g. key = "../../etc/passwd").
   *
   * Even though keys are always generated by the server as UUIDs, defense-in-depth
   * requires validating at the boundary where untrusted data could theoretically
   * reach this method (e.g. if a DB record is tampered with).
   *
   * Uses path.sep-aware comparison so it works on both Linux (/) and Windows (\).
   */
  private safeJoin(key: string): string {
    const resolved = resolve(join(this.uploadDir, key));
    const safeBase = resolve(this.uploadDir);
    // Append the platform separator so "uploadDir-other/file" is not accepted
    // when uploadDir is "uploadDir" (prefix-match false positive).
    const safeBaseWithSep = safeBase.endsWith(sep) ? safeBase : safeBase + sep;
    if (!resolved.startsWith(safeBaseWithSep) && resolved !== safeBase) {
      throw new Error(`Path traversal attempt detected: key="${key}"`);
    }
    return resolved;
  }
